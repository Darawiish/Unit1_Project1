...
############################# TO DO #####################################
    ...
    master_query = """ SELECT * FROM customers
                JOIN invoices 
    ...
    product_sum = """ SELECT product_category, SUM(unit_price*quantity)
                FROM invoices 
...
############################# TO DO ####################################
    ...
    product_sum_output = "COPY ({0}) TO STDOUT WITH CSV HEADER".format(product_sum)
...
############## CREATING CSV FILES FROM OUTPUTS #########################
    ...
    with open('product_revenue.csv', 'w') as file: 
        cursor.copy_expert(product_sum_output, file)
